    ...
    if (memberData) {
      ...
      if (!isActivated) {
        ...
        if (isActivated) {
          ...
          const { data: msDataReloaded } = await supabase.schema('kuntiy')
            .from('member_savings')
            .select('*, account:accounts(*), savings_product:saving_products(name, interest_rate)')
            .eq('member_id', memberData.id);
      ...
      let { data: productsData } = await supabase.schema('kuntiy').from('saving_products').select('*').eq('organization_id', memberData.organization_id);
      console.log('✅ Products loaded:', productsData);

      if (!productsData || productsData.length === 0) {
        console.log('🆕 No saving products found — creating a default one');
        const { data: newProduct, error } = await supabase.schema('kuntiy').from('saving_products').insert({
